...
def create_new_thread(req: CreateThreadRequest) -> CreateThreadResponse:
    ...
    global lance
    thread_doc = lance.create_thread(req.title, req.initial_message)
...
def get_thread_details(thread_id: str) -> Thread:
    ...
    global lance
    thread = lance.get_thread(thread_id)
    if not thread:
...
def get_thread_messages_endpoint(thread_id: str) -> list[Message]:
    ...
    global lance
    messages = lance.get_thread_messages(thread_id)
...
def add_message_to_thread(thread_id: str, req: AddMessageRequest) -> Message:
    ...
    global lance
    if not lance.get_thread(thread_id):
        raise HTTPException(status_code=404, detail="Thread not found")
    
    message_doc = lance.save_message(thread_id, req.role, req.content)
...
def update_thread_title(thread_id: str, req: UpdateThreadRequest) -> dict:
    ...
    lance.update_thread(thread_id, {"title": req.title})
...
def delete_thread_endpoint(thread_id: str) -> dict:
    """Delete a thread"""
    global lance
    success = lance.delete_thread(thread_id)
    if not success:
        raise HTTPException(status_code=404, detail="Thread not found")
