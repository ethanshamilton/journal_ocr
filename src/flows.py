...
def comprehensive_analysis_flow(lance: AsyncLocalLanceDB, req: ChatRequest) -> dict:
    ...
    chat_history = _load_chat_history(lance, req)
    ...
    for year in years:
        ...
        entries = lance.get_entries_by_date_range(f"{year}-01-01", f"{year}-12-31")
        ...
        for chunk in chunks:
            ...
            if len(chunks) == 1:
                analysis = comprehensive_analysis(req, [], chunk, "YEAR")
        ...
        if len(subyear_analyses) != 0:
            ...
            for analysis in subyear_analyses:
                ...
                """
            analysis = comprehensive_analysis(req, [], subyear_analyses_str, "YEAR")
            analyses.append(analysis)
            print(f"\nreasoning: {analysis.reasoning}\n\nanalysis: {analysis.analysis}\n\nexcerpts: {analysis.excerpts}\n") 
    ...
    for analysis in analyses:
        ...
        <EXCERPTS>{analysis.excerpts}</EXCERPTS>
        """
    final_analysis = comprehensive_analysis(req, chat_history, analyses_str, "FINAL")
...
def default_llm_flow(lance: AsyncLocalLanceDB, req: ChatRequest) -> ChatResponse:
    ...
    if req.existing_docs:
        ...
        if query_intent == SearchOptions.VECTOR:
            ...
            entries = lance.get_similar_entries(query_embedding, req.top_k)
            ...
            entries = lance.get_recent_entries()
    ...
    llm_response = chat_response(req, chat_history, entries_str)
...
def _load_chat_history(lance: AsyncLocalLanceDB, request: ChatRequest) -> list[dict]:
    ...
    if request.thread_id:
        try:
            thread_messages = lance.get_thread_messages(request.thread_id)
            if thread_messages:
